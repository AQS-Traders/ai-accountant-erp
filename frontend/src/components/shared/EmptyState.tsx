...
const ILLUSTRATION_BG: Record<EmptyStateIllustration, string> = {
  ...
  empty: "from-slate-100 to-slate-50",
  error: "from-red-50 to-red-100",
  success: "from-emerald-50 to-emerald-100",
  invoices: "from-blue-50 to-indigo-100",
  customers: "from-teal-50 to-cyan-100",
  suppliers: "from-amber-50 to-orange-100",
  banking: "from-violet-50 to-purple-100",
  reports: "from-indigo-50 to-blue-100",
  journal: "from-emerald-50 to-teal-100",
};
...
export default function EmptyState({
  ...
  return (
    ...
    >
      ...
      <h3 className="text-lg font-semibold text-text-primary mb-2 max-w-md">
        {title}
      </h3>
      ...
      {(primaryAction || secondaryAction || onAiClick) && (
        <div className="flex flex-wrap items-center justify-center gap-3 mb-8">
          ...
          {onAiClick && (
            <button
              onClick={onAiClick}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-ai-50 border border-ai-200 text-ai-700 text-sm font-medium hover:bg-ai-100 transition-colors"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-2xl mb-8">
          {quickActions.map((action, idx) => (
            <button
              ...
              className={cn(
                "group flex flex-col items-center gap-3 p-4 rounded-xl border transition-all hover:-translate-y-0.5",
                action.variant === "primary"
                  ? "border-ai-200 bg-ai-50 hover:bg-ai-100"
                  : "border-border-subtle bg-bg-surface hover:border-ai-200 hover:bg-bg-muted"
              )}
            >
        ...
        <div className="w-full max-w-2xl space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {quickTips.map((tip, idx) => (
              <div
                key={idx}
                className="text-left p-3 rounded-xl bg-bg-muted border border-border-subtle"
              >
                ...
                {tip.action && tip.href && (
                  <a
                    href={tip.href}
                    className="inline-flex items-center gap-1 text-xs text-ai-600 hover:text-ai-700 mt-2"
                  >
...
export function NoDataEmptyState({
  ...
  return (
    <EmptyState
      ...
      description={`You haven't created any ${
        entityLabel || entityName
      } yet. Add your first one manually or ask the AI to create it for you.`}
      primaryAction={
        onCreate
          ? { label: `Add ${entityName}`, onClick: onCreate, icon: Plus }
          : undefined
      }
      onAiClick={onAiClick}
      aiPrompt={aiContext ? `Create ${aiContext}` : `Add a ${entityName}`}
      quickTips={[
        {
          title: "Manual Entry",
          description: `Click "Add ${entityName}" to create one manually.`,
        },
        {
          title: "AI Assistant",
          description:
            "Tell the AI in plain language and it handles the accounting.",
        },
      ]}
    />
  );
...
export function ErrorEmptyState({
  ...
  description =
    "We couldn't load this data. This might be a temporary issue.",
  onRetry,
  errorCode,
}: {
  title?: string;
  ...
  return (
    <EmptyState
      ...
      primaryAction={
        onRetry
          ? { label: "Try Again", onClick: onRetry, icon: ArrowRight }
          : undefined
      }
      quickTips={
        errorCode ? [{ title: "Error Code", description: errorCode }] : undefined
      }
    />
  );
}